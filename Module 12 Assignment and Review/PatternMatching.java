...
public class PatternMatching {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern, CharSequence text, CharacterComparator comparator) {
        ...
        while (i <= n - m) {
            // index j =  every time we iterate the pattern
            int j = m - 1;
            while (j >= 0 && comparator.compare(pattern.charAt(j), text.charAt(i + j)) == 0) {
                j -= 1;
            }
            // two cases, 1: all match 2: not all match
            if (j == -1) {
                res.add(i);
                i += 1;
            } else {
                int shift = lastTable.getOrDefault(text.charAt(j + i), -1);
                if (shift == -1) { // this char not appear in the pattern
                    i += (j + 1);
                } else if (shift < j) {
                    i += (j - shift);
                } else {
    ...
    public static void main(String[] args) {
        ...
        List<Integer> lst = boyerMoore(pattern, text, cc);
